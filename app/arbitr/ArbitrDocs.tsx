...
export const CaseDataContext = createContext<ICaseDataContext>(
  {} as ICaseDataContext
);
...
const ArbitrDocs = () => {
    ...
    useEffect(() => {
    // data initialising by first arrays values
        if (isCaseData) {
            if (!applicantName && caseData.Sides.Participants[0].isActualisedByDadata) {
                setApplicantName(caseData.Sides.Participants[0].Name);
            }
            setDocTitle("Отзыв на исковое заявление");
